...
class BosePresetSelect(
    ...
    def __init__(self, coordinator_id: str, coordinator: BoseSoundTouchCoordinator) -> None:
        ...
        BosePresetMixin.__init__(self, coordinator_id, coordinator)
...
class BosePresetBinarySensor(
    ...
    def __init__(
        ...
        *,
        ...
        BosePresetMixin.__init__(self, coordinator_id, coordinator)
        self._preset_id = preset_id
        self._attr_name = f"Preset {preset_id} aktiv"
        self._attr_unique_id = (
            f"{coordinator.entry.entry_id}_{coordinator_id}_preset_binary_sensor_{preset_id}"
        )
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        preset = next((item for item in self._presets if int(item.get("id", 0)) == self._preset_id), None)
        return {
            "preset_id": self._preset_id,
            "preset_name": str(preset.get("item_name") or "").strip() if preset else "",
        }
