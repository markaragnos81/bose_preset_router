...
from .radio_browser import (
    async_fetch_icy_meta,
    async_lookup_track_art,
...
class StreamMetadataTracker:
    ...
    async def async_set_stream(self, url: str) -> dict[str, Any]:
        ...
        if self._task is None or self._task.done():
            self._task = self.hass.async_create_background_task(
                self._async_run(),
                f"bose_preset_router_stream_meta_{url}",
            )
    ...
    async def _async_build_metadata(self, url: str) -> dict[str, Any]:
        ...
        icy_meta = await async_fetch_icy_meta(self.hass, url)
        ...
        track_image = ""
        if track_title:
            track_image = await async_lookup_track_art(
                self.hass,
                artist=track_artist,
                title=track_title,
            )
        ...
        if stream_title:
            _LOGGER.debug(
                "Stream title classified for %s: classification=%s reason=%s stream_title=%r station=%r icy_name=%r artist=%r title=%r",
                url,
                "track" if track_title else "branding",
                decision_reason,
                stream_title,
                station_name,
                icy_name,
...
def _classify_stream_title(
    ...
    *,
    ...
    if _looks_like_station_branding(
        stream_title,
        station_name=station_name,
        icy_name=icy_name,
    ):
    ...
    if any(
        ...
        for token in ("listen live", "on air", "webradio", "unknown", "advert", "jingle")
    ):
